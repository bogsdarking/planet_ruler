...
class ImageSegmentation:
    ...
    def _combine_masks(self, classified_masks: dict) -> np.ndarray:
        ...
        planet_mask_filled = cv2.morphologyEx(
            planet_mask.astype(np.uint8), cv2.MORPH_CLOSE, kernel
        ).astype(bool)

        sky_mask_filled = cv2.morphologyEx(
            sky_mask.astype(np.uint8), cv2.MORPH_CLOSE, kernel
        ).astype(bool)
        ...
        if self.segmenter == "segment-anything":
            ...
            if self.interactive:
                ...
                selector = TkMaskSelector(
                    image=self.image, masks=masks, initial_zoom=0.5
                )
                selector.run()
                ...
                combined_mask = (1 - masks[0]["segmentation"]) * masks[1][
                    "segmentation"
                ]
