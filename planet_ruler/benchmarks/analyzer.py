...
class BenchmarkAnalyzer:
    ...
    def get_results(self, 
                    ...
                    max_timestamp: Optional[str] = None) -> pd.DataFrame:
        """
        ...
        # Parse JSON columns
        json_cols = ["fit_params", "free_parameters", "init_parameter_values", 
                     "parameter_limits", "best_parameters", "minimizer_config"]
    ...
    def explode_parameters(self, df: Optional[pd.DataFrame] = None) -> pd.DataFrame:
        ...
        for param in all_params:
            df[f'{param}_lower'] = df['parameter_limits'].apply(
                lambda x: x.get(param, [None, None])[0] if isinstance(x, dict) and param in x else None
            )
            df[f'{param}_upper'] = df['parameter_limits'].apply(
                lambda x: x.get(param, [None, None])[1] if isinstance(x, dict) and param in x else None
            )
        ...
        for fit_param in all_fit_params:
            ...
            if fit_param == 'resolution_stages':
                df[fit_param] = df['fit_params'].apply(
                    lambda x: str(x.get(fit_param)) if isinstance(x, dict) and fit_param in x else None
                )
            else:
                df[fit_param] = df['fit_params'].apply(
    ...
    def get_pareto_frontier(self, 
                           ...
                           error_col: str = "relative_error") -> pd.DataFrame:
        """
        ...
        for i in range(len(df)):
            if pareto_mask[i]:
                ...
                dominates = (
                    ...
                    ((df[time_col] < df[time_col].iloc[i]) | (df[error_col] < df[error_col].iloc[i]))
                )
                if dominates.any():
                    pareto_mask[i] = False
    ...
    def compare_scenarios(self, scenarios: List[str], metric: str = "total_time") -> pd.DataFrame:
        """
    ...
    def identify_bottlenecks(self, scenario: str, threshold: float = 0.5) -> Dict[str, float]:
        """
