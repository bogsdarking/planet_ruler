...
def plot_performance_vs_accuracy(df, output_dir=None):
    ...
    if output_dir:
        plt.savefig(output_dir / 'performance_vs_accuracy.png', dpi=150, bbox_inches='tight')
...
def show_pareto_frontier(analyzer):
    ...
    display_cols = ['scenario_name', 'image_name', 'total_time', 'relative_error', 'iterations']
...
def plot_runtime_distribution(df, output_dir=None):
    ...
    if output_dir:
        plt.savefig(output_dir / 'runtime_distribution.png', dpi=150, bbox_inches='tight')
...
def plot_timing_breakdown(df, output_dir=None):
    ...
    mean_times.plot(kind='bar', stacked=True, ax=ax, color=['#1f77b4', '#ff7f0e', '#2ca02c'])
...
def main():
    ...
    if len(df) == 0:
        ...
        print("  python benchmarks/run_benchmarks.py benchmarks/configs/smoke_test.yaml")
        return
