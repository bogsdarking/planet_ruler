...
Examples:
  ...
  python -m planet_ruler.benchmarks.run_benchmarks benchmarks/configs/full_suite.yaml --image pexels-claiton-17217951_exif
    ...
    if not args.quiet:
        ...
        if successful > 0:
            successful_results = [r for r in results if r.convergence_status == "success"]
            mean_time = sum(r.total_time for r in successful_results) / len(successful_results)
            mean_error = sum(r.relative_error for r in successful_results if r.relative_error is not None)
            mean_error = mean_error / len([r for r in successful_results if r.relative_error is not None])
        ...
        print("  - Export CSV: python -c 'from benchmarks.analyzer import BenchmarkAnalyzer; "
