...
def calculate_parameter_uncertainty(
    ...
    **kwargs
    ...
    if method == 'auto':
        if observation.minimizer == 'differential-evolution' and hasattr(observation.fit_results, 'population'):
            method = 'population'
        else:
...
def _uncertainty_from_hessian(
    ...
    try:
        ...
        if variance < 0:
            logging.warning(f"Negative variance for {parameter} - Hessian may be poorly conditioned")
            return {
                'uncertainty': 0.0,
                'method': 'hessian',
                'confidence_level': confidence_level,
...
def _uncertainty_from_profile(
    ...
    for param_value in param_values:
        ...
        bounds_free = [observation.parameter_limits[p] 
...
def _uncertainty_from_bootstrap(
    ...
    for i in range(n_bootstrap):
        ...
        logging.warning("Bootstrap not fully implemented - needs careful state management")
        break
