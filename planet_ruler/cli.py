...
Examples:
  ...
  planet-ruler demo --planet earth
    ...
    # Demo command
    demo_parser = subparsers.add_parser('demo', help='Run demonstration with example data')
    ...
    list_parser = subparsers.add_parser('list', help='List available example configurations')
...
def measure_command(args):
    ...
    try:
        ...
        # Display results
        print(f"\nResults:")
        print(f"  Estimated planetary radius: {obs.radius_km:.0f} ± {getattr(obs, 'radius_uncertainty', 0):.0f} km")
        ...
        if args.save_plots:
            ...
            plot_path = os.path.join(args.save_plots, 
                                   f"{Path(args.image).stem}_analysis.png")
...
def demo_command(args):
    ...
    if args.interactive:
        try:
            ...
            import subprocess
            notebook_path = Path(__file__).parent.parent / "notebooks" / "limb_demo.ipynb"
            subprocess.run(["jupyter", "notebook", str(notebook_path)], check=True)
        except ImportError:
            print("Jupyter notebook not available. Install with: pip install jupyter", file=sys.stderr)
            return 1
        ...
        try:
            ...
            if args.planet:
                ...
                print(f"Demo for {args.planet} completed. Use --interactive for full experience.")
            else:
