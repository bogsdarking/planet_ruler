...
class TestRealDemoDataIntegration:
    ...
    def test_earth_iss_horizon_calculation(self, earth_iss_config):
        ...
        assert 2500000 < horizon_dist < 2700000, f"ISS horizon distance {horizon_dist/1000:.1f} km seems unrealistic"
        
        # Calculate limb angle
        limb_angle = geom.limb_camera_angle(r=params["r"], h=params["h"])
        
        # Limb angle should be reasonable for ISS
        assert 0.1 < limb_angle < 0.4, f"ISS limb angle {limb_angle:.3f} rad seems unrealistic"
        ...
        assert abs(calculated_fov - expected_fov) < 1.0, "Field of view calculation inconsistent"
    ...
    def test_pluto_new_horizons_scenario(self, pluto_config):
        ...
        assert horizon_dist > 1000000, f"Pluto horizon distance {horizon_dist/1000:.1f} km too small"
        ...
        assert 1.0 < limb_angle < 2.0, f"Pluto limb angle {limb_angle:.6f} rad from New Horizons distance"
    ...
    def test_saturn_cassini_scenario(self, saturn_config):
        ...
        assert horizon_dist > 10000000, f"Saturn horizon distance {horizon_dist/1000:.1f} km too small"
        ...
        assert 0.1 < limb_angle < 2.0, f"Saturn limb angle {limb_angle:.6f} rad from Cassini distance"
    ...
    @pytest.mark.integration
    def test_parameter_scaling_across_scenarios(self, earth_iss_config, pluto_config, saturn_config):
        ...
        for result in results:
            assert result["altitude"] > 0, f"{result['name']} altitude should be positive"
            # Verify relative altitude (h/r) is reasonable for the observation geometry
            assert result["relative_altitude"] >= 0, f"{result['name']} relative altitude should be non-negative"
        
        # All limb angles should be reasonable positive values
        assert all(0.1 < r["limb_angle"] < 2.0 for r in results), "All limb angles should be reasonable"
    ...
    def test_cost_function_integration_earth_iss(self, earth_iss_config):
        ...
        assert cost_perfect < 0.01, f"Cost with perfect parameters should be low, got {cost_perfect}"
        ...
        assert cost_perturbed > cost_perfect, "Perturbed parameters should give higher cost"
        
        # Test parameter packing/unpacking
        packed = pack_parameters(params, params)
        unpacked = unpack_parameters(packed, list(params.keys()))
        for key in params:
            assert abs(unpacked[key] - params[key]) < 1e-10, f"Parameter {key} packing/unpacking failed"
    ...
    def test_image_processing_integration(self, mock_image_open):
        ...
        assert 180 < mean_break < 220, f"Expected horizon around y=200, got {mean_break:.1f}"
        
        # Test limb smoothing
        smoothed_limb = img.smooth_limb(breaks, method="rolling-median", window_length=15)
        assert len(smoothed_limb) == len(breaks)
        assert np.var(smoothed_limb) < np.var(breaks), "Smoothed limb should have less variance"
        ...
        assert not any(math.isnan(float(x)) for x in filled.flatten()), "All NaNs should be filled"
            ...
            with patch('numpy.array', return_value=image):
                ...
                try:
                    ...
                    if len(limb.shape) > 1:
                        ...
                        pytest.skip("Limb detection returned full image instead of limb coordinates")
                    assert len(limb) == width
                    assert 130 < np.mean(limb) < 170, "Limb should be detected around horizon"
    ...
    def test_demo_parameter_loading_integration(self):
        ...
        for scenario in demo_scenarios:
            try:
                ...
                if scenario == "earth":
                    assert 6e6 < params["r"] < 7e6, f"Earth radius unrealistic: {params['r']}"
                elif scenario == "pluto":  
                    assert 5e5 < params["r"] < 2e6, f"Pluto radius unrealistic: {params['r']}"
                elif scenario.startswith("saturn"):
                    assert 5e7 < params["r"] < 8e7, f"Saturn radius unrealistic: {params['r']}"
    ...
    def test_coordinate_transform_integration(self, saturn_config):
        ...
        transformed = geom.intrinsic_transform(
            test_coords_intrinsic,
            f=params["f"],
            x0=250,
            y0=150
        )
        # Transform reduces from 4D to 3D homogeneous coordinates
        assert transformed.shape[1] == test_coords_intrinsic.shape[1]  # Same number of points
        ...
        assert extrinsic.shape[1] == test_coords_extrinsic.shape[0]  # Same number of points
...
class TestConfigurationConsistency:
    ...
    def test_all_configs_have_required_fields(self):
        ...
        required_fields = ["free_parameters", "init_parameter_values", "parameter_limits"]
        ...
        for config_file in self.config_dir.glob("*.yaml"):
            ...
            for param in config["free_parameters"]:
                if param in init_vals and param in limits:
                    ...
                    assert low <= init_val <= high, f"Initial value {init_val} for {param} outside limits [{low}, {high}] in {config_file.name}"
    ...
    def test_physical_parameter_reasonableness(self):
        ...
        for config_file in self.config_dir.glob("*.yaml"):
            ...
            if "r" in params:
                assert 1e5 < params["r"] < 1e8, f"Unrealistic radius {params['r']} in {config_file.name}"
            ...
            if "fov" in params:
                assert 0.1 < params["fov"] < 180, f"Unrealistic FOV {params['fov']} in {config_file.name}"