...
from planet_ruler.plot import (
    plot_image, plot_limb, plot_3d_solution, plot_topography,
    plot_gradient_field_at_limb, compare_blur_methods, compare_gradient_fields,
    plot_diff_evol_posteriors, plot_full_limb, plot_segmentation_masks
)
...
class TestPlotGradientFieldAtLimb:
    ...
    def test_plot_gradient_field_edge_cases(self, mock_plt, mock_gradient_field):
        ...
        y_pixels = np.array([15, 16, 17])  # Minimum length to avoid gradient calculation issues
...
class TestCompareBlurMethods:
    ...
    @patch("planet_ruler.plot.cv2")
    @patch("planet_ruler.plot.plt")
    def test_compare_blur_methods_with_limb(self, mock_plt, mock_cv2, mock_gradient_field):
    ...
    @patch("planet_ruler.plot.cv2")
    @patch("planet_ruler.plot.plt")
    def test_compare_blur_methods_grayscale(self, mock_plt, mock_cv2, mock_gradient_field):
...
class TestCompareGradientFields:
    ...
    def test_compare_gradient_fields_multiple_limbs(self, mock_line_collection, mock_plt, mock_gradient_field):
        ...
        y_pixels_list = [
            np.array([25, 26, 27, 26, 25]),
            np.array([30, 31, 32, 31, 30])
        ]
    ...
    @patch("planet_ruler.plot.plt")
    @patch("matplotlib.collections.LineCollection")
    def test_compare_gradient_fields_single_limb(self, mock_line_collection, mock_plt, mock_gradient_field):
        ...
        mock_fig = Mock()
        mock_ax = Mock()
        mock_plt.subplots.return_value = (mock_fig, mock_ax)  # Single axis, will be wrapped by function
        
        y_pixels_list = [np.array([20, 21, 22, 21, 20])]
        labels = ["Single Limb"]
        image = np.random.random((40, 50, 3))
        
        compare_gradient_fields(y_pixels_list, labels, image,
                               gradient_smoothing=3.0, streak_length=25, decay_rate=0.2)
...
class TestPlotDiffEvolPosteriors:
    ...
    @patch("planet_ruler.plot.plt")
    @patch("planet_ruler.plot.sns")
    def test_plot_diff_evol_posteriors_missing_init_values(self, mock_sns, mock_plt, mock_unpack):
...
class TestPlotFullLimb:
    ...
    def test_plot_full_limb_with_init_parameters(self, mock_plt, mock_limb_arc):
        ...
        mock_obs.init_parameter_values = {"r": 3390000, "h": 200000, "cx": 400, "cy": 250}
...
class TestPlotSegmentationMasks:
    ...
    def test_plot_segmentation_masks_single_mask(self, mock_plt):
        ...
        mock_obs = Mock()
        mock_obs._segmenter._masks = [
            {"segmentation": np.random.random((50, 75))}
        ]
